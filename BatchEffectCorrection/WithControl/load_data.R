...
# load ROI metadata
roi_meta_path <- file.path(data_root, "Metadata", "WithControlMetaIMC.csv")
...
roi_fea_dir <- file.path(data_root, "WithControl", "CellFeas")
np <- import("numpy")
p_len <- 2
p_num <- 35
...
# add batch info
batch_ids <-
    roi_metadata[["Batch"]][match(fea_filenames, roi_metadata[["Filename"]])] %>%
    as.factor() %>% rep(roi_nrows)
roi_feas$batch <- batch_ids
# add condition info
condition_ids <-
    roi_metadata[["Condition"]][match(fea_filenames, roi_metadata[["Filename"]])] %>%
    as.factor() %>% rep(roi_nrows)
roi_feas$condition <- condition_ids

anchor_ids <- 
    roi_metadata[["Anchor"]][match(fea_filenames, roi_metadata[["Filename"]])] %>%
    as.factor() %>% rep(roi_nrows)
roi_feas$anchor <- anchor_ids
