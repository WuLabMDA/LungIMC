...
if __name__ == "__main__":
    ...
    for cur_fea_name in roi_fea_names:
        ...
        # max_mean_rest = np.max([mean_normal, mean_aah, mean_mia, mean_adc])
        # if mean_ais > max_mean_rest:
        ais_normal_p = stats.ttest_ind(ais_feas, normal_feas)
        ais_aah_p = stats.ttest_ind(ais_feas, aah_feas)
        ais_mia_p = stats.ttest_ind(ais_feas, mia_feas)
        ais_adc_p = stats.ttest_ind(ais_feas, adc_feas)

        max_pval = np.max([ais_normal_p.pvalue, ais_aah_p.pvalue, ais_mia_p.pvalue, ais_adc_p.pvalue])
        if max_pval < args.pval_thresh:
            feature_fc_dict[cur_fea_name] = mean_ais
    sorted_fea_dict = dict(sorted(feature_fc_dict.items(), key=operator.itemgetter(1),reverse=True))
    print("AIS has {} significant features.".format(len(sorted_fea_dict)))
