...
if __name__ == "__main__":
    ...
    for cur_fea_name in roi_fea_names:
        ...
        # max_mean_rest = np.max([mean_normal, mean_ais, mean_mia, mean_adc])
        # if mean_aah > max_mean_rest:
        aah_normal_p = stats.ttest_ind(aah_feas, normal_feas)
        aah_ais_p = stats.ttest_ind(aah_feas, ais_feas)
        aah_mia_p = stats.ttest_ind(aah_feas, mia_feas)
        aah_adc_p = stats.ttest_ind(aah_feas, adc_feas)
        max_pval = np.max([aah_normal_p.pvalue, aah_ais_p.pvalue, aah_mia_p.pvalue, aah_adc_p.pvalue])
        if max_pval < args.pval_thresh:
            feature_fc_dict[cur_fea_name] = mean_aah

    sorted_fea_dict = dict(sorted(feature_fc_dict.items(), key=operator.itemgetter(1),reverse=True))
    print("AAH has {} significant features.".format(len(sorted_fea_dict)))
