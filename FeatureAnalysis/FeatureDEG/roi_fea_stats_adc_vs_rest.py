...
if __name__ == "__main__":
    ...
    for cur_fea_name in roi_fea_names:
        ...
        max_mean_rest = np.max([mean_normal, mean_aah, mean_aismia])
        if mean_adc > max_mean_rest:
            adc_normal_p = stats.ttest_ind(adc_feas, normal_feas)
            adc_aah_p = stats.ttest_ind(adc_feas, aah_feas)
            adc_aismia_p = stats.ttest_ind(adc_feas, aismia_feas)
            max_pval = np.max([adc_normal_p.pvalue, adc_aah_p.pvalue, adc_aismia_p.pvalue])
            if max_pval < args.pval_thresh:
                feature_lst.append(cur_fea_name)
    print("ADC has {} DEG significant features.".format(len(feature_lst)))
    print(feature_lst)