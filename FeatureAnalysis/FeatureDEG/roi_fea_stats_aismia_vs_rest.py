...
if __name__ == "__main__":
    ...
    for cur_fea_name in roi_fea_names:
        ...
        max_mean_rest = np.max([mean_normal, mean_aah, mean_adc])
        if mean_aismia > max_mean_rest:
            aismia_normal_p = stats.ttest_ind(aismia_feas, normal_feas)
            aismia_aah_p = stats.ttest_ind(aismia_feas, aah_feas)
            aismia_adc_p = stats.ttest_ind(aismia_feas, adc_feas)
            max_pval = np.max([aismia_normal_p.pvalue, aismia_aah_p.pvalue, aismia_adc_p.pvalue])
            if max_pval < args.pval_thresh:
                feature_lst.append(cur_fea_name)
    print("AISMIA has {} DEG significant features.".format(len(feature_lst)))
    print(feature_lst)