...
for (cell_type in all_cell_lst) {
    ...
    for (ir in 1:length(aah_roi_lst)) {
        ...
        cur_roi_area <- aah_filter_df$Area[ir]
        roi_cell_density <- sum(cur_roi_cell_types == cell_type) / cur_roi_area
        density_lst <- append(density_lst, roi_cell_density)
        location_lst <- append(location_lst, aah_filter_df$Core_Margin[ir])
    }
    
    cell_density_df <- data.frame(Density=density_lst, Loc=location_lst)
    long_density_df <- gather(cell_density_df, key = "variable", value = "value", -Density) 
    
    # Distant vs Adjacent
    dist_adja_ttest <- t.test(long_density_df$Density[long_density_df$value=="DistantNormal"], long_density_df$Density[long_density_df$value=="AdjacentNormal"])
    aah_dist_adja_pvals <- append(aah_dist_adja_pvals, dist_adja_ttest$p.value)
    if (mean(long_density_df$Density[long_density_df$value=="AdjacentNormal"]) >= mean(long_density_df$Density[long_density_df$value=="DistantNormal"]))
        aah_dist_adja_cmps <- append(aah_dist_adja_cmps, TRUE)
    else
        aah_dist_adja_cmps <- append(aah_dist_adja_cmps, FALSE)   
    # Distant vs AAH
    dist_aah_ttest <- t.test(long_density_df$Density[long_density_df$value=="DistantNormal"], long_density_df$Density[long_density_df$value=="AAH"])
    aah_dist_aah_pvals <- append(aah_dist_aah_pvals, dist_aah_ttest$p.value)
    if (mean(long_density_df$Density[long_density_df$value=="AAH"]) >= mean(long_density_df$Density[long_density_df$value=="DistantNormal"]))
        aah_dist_aah_cmps <- append(aah_dist_aah_cmps, TRUE)
    else
        aah_dist_aah_cmps <- append(aah_dist_aah_cmps, FALSE)      
    # Adjacent vs AAH
    adja_aah_ttest <- t.test(long_density_df$Density[long_density_df$value=="AdjacentNormal"], long_density_df$Density[long_density_df$value=="AAH"])
    aah_adja_aah_pvals <- append(aah_adja_aah_pvals, adja_aah_ttest$p.value)
    if (mean(long_density_df$Density[long_density_df$value=="AAH"]) >= mean(long_density_df$Density[long_density_df$value=="AdjacentNormal"]))
    ...
    for (ir in 1:length(ais_roi_lst)) {
        ...
        cur_roi_cell_types <- celltype_lst[cur_roi_cell_indices]
        cur_roi_area <- ais_filter_df$Area[ir]
        roi_cell_density <- sum(cur_roi_cell_types == cell_type) / cur_roi_area
        density_lst <- append(density_lst, roi_cell_density)
    ...
    # Distant vs Margin
    dist_marg_ttest <- t.test(long_density_df$Density[long_density_df$value=="DistantNormal"], long_density_df$Density[long_density_df$value=="Margin"])
    ais_dist_marg_pvals <- append(ais_dist_marg_pvals, dist_marg_ttest$p.value)
    if (mean(long_density_df$Density[long_density_df$value=="Margin"]) >= mean(long_density_df$Density[long_density_df$value=="DistantNormal"]))
        ais_dist_marg_cmps <- append(ais_dist_marg_cmps, TRUE)
    else
        ais_dist_marg_cmps <- append(ais_dist_marg_cmps, FALSE)
    # Distant vs Core
    dist_core_ttest <- t.test(long_density_df$Density[long_density_df$value=="DistantNormal"], long_density_df$Density[long_density_df$value=="Core"])
    ais_dist_core_pvals <- append(ais_dist_core_pvals, dist_core_ttest$p.value)
    if (mean(long_density_df$Density[long_density_df$value=="Core"]) >= mean(long_density_df$Density[long_density_df$value=="DistantNormal"]))
        ais_dist_core_cmps <- append(ais_dist_core_cmps, TRUE)
    else
        ais_dist_core_cmps <- append(ais_dist_core_cmps, FALSE)
    # Adjacent vs Margin
    adja_marg_ttest <- t.test(long_density_df$Density[long_density_df$value=="AdjacentNormal"], long_density_df$Density[long_density_df$value=="Margin"])
    ais_adja_marg_pvals <- append(ais_adja_marg_pvals, adja_marg_ttest$p.value)
    if (mean(long_density_df$Density[long_density_df$value=="Margin"]) >= mean(long_density_df$Density[long_density_df$value=="AdjacentNormal"]))
        ais_adja_marg_cmps <- append(ais_adja_marg_cmps, TRUE)
    else
        ais_adja_marg_cmps <- append(ais_adja_marg_cmps, FALSE)
    # Adjacent vs Core
    adja_core_ttest <- t.test(long_density_df$Density[long_density_df$value=="AdjacentNormal"], long_density_df$Density[long_density_df$value=="Core"])
    ais_adja_core_pvals <- append(ais_adja_core_pvals, adja_core_ttest$p.value)
    if (mean(long_density_df$Density[long_density_df$value=="Core"]) >= mean(long_density_df$Density[long_density_df$value=="AdjacentNormal"]))
        ais_adja_core_cmps <- append(ais_adja_core_cmps, TRUE)
    else
        ais_adja_core_cmps <- append(ais_adja_core_cmps, FALSE)
    # Margin vs Core
    marg_core_ttest <- t.test(long_density_df$Density[long_density_df$value=="Margin"], long_density_df$Density[long_density_df$value=="Core"])
    ais_marg_core_pvals <- append(ais_marg_core_pvals, marg_core_ttest$p.value)
    if (mean(long_density_df$Density[long_density_df$value=="Core"]) >= mean(long_density_df$Density[long_density_df$value=="Margin"]))
    ...
    for (ir in 1:length(mia_roi_lst)) {
        ...
        cur_roi_cell_types <- celltype_lst[cur_roi_cell_indices]
        cur_roi_area <- mia_filter_df$Area[ir]
        roi_cell_density <- sum(cur_roi_cell_types == cell_type) / cur_roi_area
        density_lst <- append(density_lst, roi_cell_density)
    ...
    for (ir in 1:length(adc_roi_lst)) {
        ...
        cur_roi_cell_types <- celltype_lst[cur_roi_cell_indices]
        cur_roi_area <- adc_filter_df$Area[ir]
        roi_cell_density <- sum(cur_roi_cell_types == cell_type) / cur_roi_area
        density_lst <- append(density_lst, roi_cell_density)
