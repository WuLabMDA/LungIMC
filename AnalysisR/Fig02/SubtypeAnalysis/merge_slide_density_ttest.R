...
for (cell_type in all_cell_lst) {
    ...
    aah_tmb_density_lst <- c()
    aah_tmb_status_lst <- c()
    ais_tmb_density_lst <- c()
    ais_tmb_status_lst <- c()
    mia_tmb_density_lst <- c()
    mia_tmb_status_lst <- c()
    adc_tmb_density_lst <- c()
    adc_tmb_status_lst <- c()
    
    mia_egfr_kras_density_lst <- c()
    ...
    for (ir in 1:length(stage_slide_lst)) {
        ...
        if (!is.na(cur_tmb)) {
            ...
            if (cur_diag == "AIS") {
                ais_tmb_density_lst <- append(ais_tmb_density_lst, slide_cell_density)
                ais_tmb_status_lst <- append(ais_tmb_status_lst, cur_tmb)
            }
            if (cur_diag == "MIA") {
                mia_tmb_density_lst <- append(mia_tmb_density_lst, slide_cell_density)
                mia_tmb_status_lst <- append(mia_tmb_status_lst, cur_tmb)
            }
            if (cur_diag == "ADC") {
                adc_tmb_density_lst <- append(adc_tmb_density_lst, slide_cell_density)
                adc_tmb_status_lst <- append(adc_tmb_status_lst, cur_tmb)
            }            
        }
        if (!is.na(cur_egfr_kras)) {
            if (cur_diag == "MIA") {
                mia_egfr_kras_density_lst <- append(mia_egfr_kras_density_lst, slide_cell_density)
                mia_egfr_kras_status_lst <- append(mia_egfr_kras_status_lst, cur_egfr_kras)
            }
            if (cur_diag == "ADC") {
                adc_egfr_kras_density_lst <- append(adc_egfr_kras_density_lst, slide_cell_density)
    ...
    age_ttest <- t.test(long_density_df$Density[long_density_df$value=="<=71"], long_density_df$Density[long_density_df$value==">71"])
    age_pvals <- append(age_pvals, age_ttest$p.value)
    if (mean(long_density_df$Density[long_density_df$value==">71"]) >= mean(long_density_df$Density[long_density_df$value=="<=71"]))
        age_cmps <- append(age_cmps, TRUE)
    else
        age_cmps <- append(age_cmps, FALSE)
    gender_ttest <- t.test(long_density_df$Density[long_density_df$value=="F"], long_density_df$Density[long_density_df$value=="M"])
    gender_pvals <- append(gender_pvals, gender_ttest$p.value)
    if (mean(long_density_df$Density[long_density_df$value=="F"]) >= mean(long_density_df$Density[long_density_df$value=="M"]))
        gender_cmps <- append(gender_cmps, TRUE)
    else
        gender_cmps <- append(gender_cmps, FALSE)
    race_ttest <- t.test(long_density_df$Density[long_density_df$value=="White"], long_density_df$Density[long_density_df$value=="Asian"])
    race_pvals <- append(race_pvals, race_ttest$p.value)
    if (mean(long_density_df$Density[long_density_df$value=="White"]) >= mean(long_density_df$Density[long_density_df$value=="Asian"]))
        race_cmps <- append(race_cmps, TRUE)
    else
        race_cmps <- append(race_cmps, FALSE)    
    smoke_ttest <- t.test(long_density_df$Density[long_density_df$value=="Non-Smoker"], long_density_df$Density[long_density_df$value=="Smoker"])
    smoke_pvals <- append(smoke_pvals, smoke_ttest$p.value)
    if (mean(long_density_df$Density[long_density_df$value=="Smoker"]) >= mean(long_density_df$Density[long_density_df$value=="Non-Smoker"]))
    ...
    if (mean(long_density_df$Density[long_density_df$value=="AAH"]) >= mean(long_density_df$Density[long_density_df$value=="Normal"]))
    ...
    if (mean(long_density_df$Density[long_density_df$value=="AIS"]) >= mean(long_density_df$Density[long_density_df$value=="Normal"]))
    ...
    if (mean(long_density_df$Density[long_density_df$value=="MIA"]) >= mean(long_density_df$Density[long_density_df$value=="Normal"]))
    ...
    if (mean(long_density_df$Density[long_density_df$value=="ADC"]) >= mean(long_density_df$Density[long_density_df$value=="Normal"]))
    ...
    if (mean(long_density_df$Density[long_density_df$value=="AIS"]) >= mean(long_density_df$Density[long_density_df$value=="AAH"]))
    ...
    if (mean(long_density_df$Density[long_density_df$value=="MIA"]) >= mean(long_density_df$Density[long_density_df$value=="AAH"]))
    ...
    if (mean(long_density_df$Density[long_density_df$value=="ADC"]) >= mean(long_density_df$Density[long_density_df$value=="AAH"]))
    ...
    if (mean(long_density_df$Density[long_density_df$value=="MIA"]) >= mean(long_density_df$Density[long_density_df$value=="AIS"]))
    ...
    if (mean(long_density_df$Density[long_density_df$value=="ADC"]) >= mean(long_density_df$Density[long_density_df$value=="AIS"]))
    ...
    if (mean(long_density_df$Density[long_density_df$value=="ADC"]) >= mean(long_density_df$Density[long_density_df$value=="MIA"]))
    ...
    ## AAH TMB
    cell_density_df <- data.frame(Density=aah_tmb_density_lst, TMB=aah_tmb_status_lst)    
    long_density_df <- gather(cell_density_df, key = "variable", value = "value", -Density) 
    aah_tmb_ttest <- t.test(long_density_df$Density[long_density_df$value=="Low"], long_density_df$Density[long_density_df$value=="High"])
    ...
    ## AIS TMB
    cell_density_df <- data.frame(Density=ais_tmb_density_lst, TMB=ais_tmb_status_lst)    
    long_density_df <- gather(cell_density_df, key = "variable", value = "value", -Density) 
    ais_tmb_ttest <- t.test(long_density_df$Density[long_density_df$value=="Low"], long_density_df$Density[long_density_df$value=="High"])
    ...
    ## MIA TMB
    cell_density_df <- data.frame(Density=mia_tmb_density_lst, TMB=mia_tmb_status_lst)    
    long_density_df <- gather(cell_density_df, key = "variable", value = "value", -Density) 
    mia_tmb_ttest <- t.test(long_density_df$Density[long_density_df$value=="Low"], long_density_df$Density[long_density_df$value=="High"])
    ...
    ## ADC TMB
    cell_density_df <- data.frame(Density=adc_tmb_density_lst, TMB=adc_tmb_status_lst)    
    long_density_df <- gather(cell_density_df, key = "variable", value = "value", -Density) 
    adc_tmb_ttest <- t.test(long_density_df$Density[long_density_df$value=="Low"], long_density_df$Density[long_density_df$value=="High"])
    ...
    ## MIA Mutation
    cell_density_df <- data.frame(Density=mia_egfr_kras_density_lst, EGFR_KRAS=mia_egfr_kras_status_lst)    
    long_density_df <- gather(cell_density_df, key = "variable", value = "value", -Density) 
    mia_egfr_kras_ttest <- t.test(long_density_df$Density[long_density_df$value=="EGFR"], long_density_df$Density[long_density_df$value=="KRAS"])
    mia_egfr_kras_pvals <- append(mia_egfr_kras_pvals, mia_egfr_kras_ttest$p.value)
    if (mean(long_density_df$Density[long_density_df$value=="EGFR"]) >= mean(long_density_df$Density[long_density_df$value=="KRAS"]))
        mia_egfr_kras_cmps <- append(mia_egfr_kras_cmps, TRUE)
    else
        mia_egfr_kras_cmps <- append(mia_egfr_kras_cmps, FALSE)      
    mia_egfr_other_ttest <- t.test(long_density_df$Density[long_density_df$value=="EGFR"], long_density_df$Density[long_density_df$value=="Other"])
    mia_egfr_other_pvals <- append(mia_egfr_other_pvals, mia_egfr_other_ttest$p.value)
    if (mean(long_density_df$Density[long_density_df$value=="EGFR"]) >= mean(long_density_df$Density[long_density_df$value=="Other"]))
        mia_egfr_other_cmps <- append(mia_egfr_other_cmps, TRUE)
    else
        mia_egfr_other_cmps <- append(mia_egfr_other_cmps, FALSE)  
    mia_kras_other_ttest <- t.test(long_density_df$Density[long_density_df$value=="KRAS"], long_density_df$Density[long_density_df$value=="Other"])
    ...
    ## MIA Mutation
    cell_density_df <- data.frame(Density=adc_egfr_kras_density_lst, EGFR_KRAS=adc_egfr_kras_status_lst)    
    long_density_df <- gather(cell_density_df, key = "variable", value = "value", -Density) 
    adc_egfr_kras_ttest <- t.test(long_density_df$Density[long_density_df$value=="EGFR"], long_density_df$Density[long_density_df$value=="KRAS"])
    ...
    adc_egfr_other_ttest <- t.test(long_density_df$Density[long_density_df$value=="EGFR"], long_density_df$Density[long_density_df$value=="Other"])
    ...
    adc_kras_other_ttest <- t.test(long_density_df$Density[long_density_df$value=="KRAS"], long_density_df$Density[long_density_df$value=="Other"])
...
p_val_df <- data.frame(Age=age_pvals, Gender=gender_pvals, Race=race_pvals, Smoke=smoke_pvals,
                       ...
                       AAH_Size=aah_size_pvals, AIS_Size=ais_size_pvals, MIA_Size=mia_size_pvals, ADC_Size=adc_size_pvals, row.names=all_cell_lst)
...
#     theme(axis.text.x = element_text(angle = 90, hju