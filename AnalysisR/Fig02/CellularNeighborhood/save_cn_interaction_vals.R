...
for (nn in 1:num_roi) {
    ...
    if (length(missing_inds) > 0) {
        ind_combs <- tidyr::crossing(missing_inds, missing_inds)
...
interaction_sigval_csv_path <- file.path(celltype_delaunay_dir, "DelaunayCN8Interaction-Sigval.csv")
