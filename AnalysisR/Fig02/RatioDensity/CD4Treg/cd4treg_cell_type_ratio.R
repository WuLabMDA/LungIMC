...
major_proportion_dir <- file.path(data_root_dir, "NatureFigures", "Fig02", "ProportionDensity", "MajorProportion")
if (!file.exists(major_proportion_dir))
    dir.create(major_proportion_dir, recursive = TRUE)

# list all cell types
cd4treg_cell_lst <- c("CD4-T-Cell",  "T-Reg-Cell")
...
for (ir in 1:length(stage_slide_lst)) {
    ...
    slide_cell_ratio <- sum(slide_celltypes %in% cd4treg_cell_lst) / length(slide_celltypes)
...
ggplot(cell_ratio_df, aes(x = Stage, y = Ratio, colour = Stage)) + 
    ...
    ggtitle(paste0("CD4Treg", " - Proportion"))

major_proportion_plot_path <- file.path(major_proportion_dir, paste0("CD4Treg", ".pdf"))
ggsave(filename = major_proportion_plot_path, device='pdf', width=5, height=8, dpi=300)
while (!is.null(dev.list()))
    dev.off()    

major_proportion_pfile <- file(file.path(major_proportion_dir, paste0("CD4Treg", "-pval.txt")), "w")
