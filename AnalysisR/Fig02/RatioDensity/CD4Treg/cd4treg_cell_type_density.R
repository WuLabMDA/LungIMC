...
for (ir in 1:length(stage_slide_lst)) {
    ...
    slide_cell_density <- sum(slide_celltypes %in% cd4treg_cell_lst) * 1000000 / slide_roi_area
    # gather information
...
ggplot(cell_density_df, aes(x = Stage, y = Density, colour = Stage)) + 
    ...
    ggtitle(paste0("CD4Treg", " - Density"))

major_density_plot_path <- file.path(major_density_dir, paste0("CD4Treg", ".pdf"))
ggsave(filename = major_density_plot_path, device='pdf', width=5, height=8, dpi=300)
while (!is.null(dev.list()))
    dev.off()        

major_density_pfile <- file(file.path(major_density_dir, paste0("CD4Treg", "-pval.txt")), "w")
...
writeLines(paste0("Normal-AAH:", normal_aah_ttest$p.value), major_density_pfile)
...
writeLines(paste0("Normal-AIS:", normal_ais_ttest$p.value), major_density_pfile)
...
writeLines(paste0("Normal-MIA:", normal_mia_ttest$p.value), major_density_pfile)
