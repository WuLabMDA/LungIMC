...
immune_proportion_dir <- file.path(data_root_dir, "NatureFigures", "Fig02", "ProportionDensity", "ImmuneProportion")
if (!file.exists(immune_proportion_dir))
    dir.create(immune_proportion_dir, recursive = TRUE)

# list all immune cell types
immune_cell_lst <- c("CD4-T-Cell", "CD8-T-Cell", "T-Reg-Cell", "B-Cell", "Macrophage", 
                         "Monocyte", "Dendritic-Cell", "Neutrophil", "MDSC", "NK-Cell")

for (cell_type in immune_cell_lst) {
    # collect information
    ratio_lst <- c()
    stage_lst <- c()
    ...
    for (ir in 1:length(stage_slide_lst)) {
        ...
        slide_immune_types <- slide_celltypes[slide_celltypes %in% immune_cell_lst]
        slide_cell_ratio <- sum(slide_immune_types == cell_type) / length(slide_immune_types)
    ...
    immune_proportion_plot_path <- file.path(immune_proportion_dir, paste0(cell_type, ".pdf"))
    ggsave(filename = immune_proportion_plot_path, device='pdf', width=5, height=8, dpi=300)
    while (!is.null(dev.list()))
        dev.off()    
    
    immune_proportion_pfile <- file(file.path(immune_proportion_dir, paste0(cell_type, "-pval.txt")), "w")
