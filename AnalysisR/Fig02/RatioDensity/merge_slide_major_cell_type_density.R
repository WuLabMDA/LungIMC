...
for (cell_type in all_cell_lst) {
    ...
    major_density_plot_path <- file.path(major_density_dir, paste0(cell_type, ".pdf"))
    ggsave(filename = major_density_plot_path, device='pdf', width=5, height=8, dpi=300)
    while (!is.null(dev.list()))
        dev.off()        
    
    major_density_pfile <- file(file.path(major_density_dir, paste0(cell_type, "-pval.txt")), "w")
