...
m1_cell_types <- c("Cytotoxic CD8 T-Cells")
# collect information
ratio_lst <- c()
stage_lst <- c()
...
ggplot(cell_ratio_df, aes(x = Stage, y = Ratio, colour = Stage)) + 
  ...
  ggtitle("Treg-CytotoxicT-Ratio")

subtype_proportion_plot_path <- file.path(subtype_proportion_dir, paste0("Treg-CytotoxicT-Ratio", ".pdf"))
ggsave(filename = subtype_proportion_plot_path, device='pdf', width=5, height=8, dpi=300)
while (!is.null(dev.list()))
  dev.off()    

subtype_proportion_pfile <- file(file.path(subtype_proportion_dir, paste0("Treg-CytotoxicT-Ratio", "-pval.txt")), "w")
