...
m2_cell_types <- c("Naive CD8 T-Cells", "Ki67+ CD8 T-Cells", "Exhausted CD8 T-Cells", "Memory CD8 T-Cells", "Cytotoxic CD8 T-Cells")
m1_cell_types <- c("Ki67- Treg-Cells", "Ki67+ Treg-Cells")
# collect information
ratio_lst <- c()
stage_lst <- c()
...
ggplot(cell_ratio_df, aes(x = Stage, y = Ratio, colour = Stage)) + 
  ...
  ggtitle("CD8T-Treg-Ratio")

subtype_proportion_plot_path <- file.path(subtype_proportion_dir, paste0("CD8T-Treg-Ratio", ".pdf"))
ggsave(filename = subtype_proportion_plot_path, device='pdf', width=5, height=8, dpi=300)
while (!is.null(dev.list()))
  dev.off()    

subtype_proportion_pfile <- file(file.path(subtype_proportion_dir, paste0("CD8T-Treg-Ratio", "-pval.txt")), "w")
