...
m2_cell_types <- c("Cytotoxic CD8 T-Cells")
...
ggplot(cell_ratio_df, aes(x = Stage, y = Ratio, colour = Stage)) + 
  ...
  ggtitle("CytotoxicT-Treg-Ratio")

subtype_proportion_plot_path <- file.path(subtype_proportion_dir, paste0("CytotoxicT-Treg-Ratio", ".pdf"))
ggsave(filename = subtype_proportion_plot_path, device='pdf', width=5, height=8, dpi=300)
while (!is.null(dev.list()))
  dev.off()    

subtype_proportion_pfile <- file(file.path(subtype_proportion_dir, paste0("CytotoxicT-Treg-Ratio", "-pval.txt")), "w")
