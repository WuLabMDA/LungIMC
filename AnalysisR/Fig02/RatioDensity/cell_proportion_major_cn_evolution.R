...
for (ind in 1:nrow(roi_df)) {
    ...
    if (roi_location %in% c("Normal", "Tumor")) 
        interested_roi_vec <- append(interested_roi_vec, roi_name)
}
...
cell_ratio_lst <- c()
# list all cell types
interested_cell_lst <- c(1, 2, 3, 4, 5, 6, 7, 8)
...
# update type name
cell_name_lst[cell_name_lst == 1] <- "Undefined-CN1"
cell_name_lst[cell_name_lst == 2] <- "Epithelial1-CN2"
cell_name_lst[cell_name_lst == 3] <- "Proliferating-CN3"
cell_name_lst[cell_name_lst == 4] <- "Epithelial2-CN4"
cell_name_lst[cell_name_lst == 5] <- "Endothelial-CN5"
cell_name_lst[cell_name_lst == 6] <- "Fibroblast-CN6"
cell_name_lst[cell_name_lst == 7] <- "Macrophage-CN7"
cell_name_lst[cell_name_lst == 8] <- "Pan-Immune-CN8"

proportion_evolution_dir <- file.path(data_root_dir, "NatureFigures", "Fig02", "ProportionDensity", "ProportionEvolution")
if (!file.exists(proportion_evolution_dir))
    dir.create(proportion_evolution_dir, recursive = TRUE)
stage_cell_ratio_df <- data.frame(Stage=stage_name_lst, CellType=cell_name_lst, CellRatio=cell_ratio_lst)
stage_cell_ratio_path <- file.path(proportion_evolution_dir, "stage_major_cn_ratios.csv")
write.csv(stage_cell_ratio_df, stage_cell_ratio_path, row.names=FALSE)
