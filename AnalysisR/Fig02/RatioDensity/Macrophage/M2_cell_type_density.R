...
subtype_density_dir <- file.path(data_root_dir, "NatureFigures", "Fig02", "ProportionDensity", "SubtypeDensityMM2")
...
for (ir in 1:length(stage_slide_lst)) {
    ...
    slide_cell_density <- sum(slide_celltypes %in% m2_cell_types) * 1000000 / slide_roi_area
    # gather information
...
ggplot(cell_density_df, aes(x = Stage, y = Density, colour = Stage)) + 
    ...
    ggtitle(paste0("M2", " - Density"))

subtype_density_plot_path <- file.path(subtype_density_dir, paste0("M2", ".pdf"))
ggsave(filename = subtype_density_plot_path, device='pdf', width=5, height=8, dpi=300)
while (!is.null(dev.list()))
    dev.off()        

subtype_density_pfile <- file(file.path(subtype_density_dir, paste0("M2", "-pval.txt")), "w")
