...
for (ir in 1:length(stage_slide_lst)) {
    ...
    slide_cell_ratio <- sum(slide_celltypes %in% m2_cell_types) / length(slide_celltypes)
...
ggplot(cell_ratio_df, aes(x = Stage, y = Ratio, colour = Stage)) + 
    ...
    ggtitle(paste0("M2", " - Proportion"))

subtype_proportion_plot_path <- file.path(subtype_proportion_dir, paste0("M2", ".pdf"))
ggsave(filename = subtype_proportion_plot_path, device='pdf', width=5, height=8, dpi=300)
while (!is.null(dev.list()))
    dev.off()    

subtype_proportion_pfile <- file(file.path(subtype_proportion_dir, paste0("M2", "-pval.txt")), "w")
