...
subtype_proportion_dir <- file.path(data_root_dir, "NatureFigures", "Fig02", "ProportionDensity", "DendriticCellProportion")
if (!file.exists(subtype_proportion_dir))
    dir.create(subtype_proportion_dir, recursive = TRUE)

# list all cell subtypes
interested_cell_lst <- c("HLADR+ Dendritic Cells", "Ki67+ Dendritic Cells", "Other Dendritic Cells")
for (cell_type in interested_cell_lst) {
    ...
    cell_ratio_df <- data.frame(Proportion=ratio_lst, Stage=stage_lst)
    long_ratio_df <- gather(cell_ratio_df, key = "variable", value = "value", -Proportion)
    
    cell_ratio_df$Stage <- factor(cell_ratio_df$Stage, levels = c("Normal", "AAH", "AIS", "MIA", "ADC"))
    ggplot(cell_ratio_df, aes(x = Stage, y = Proportion, colour = Stage)) + 
        geom_boxplot() +
    ...
    normal_aah_ttest <- t.test(long_ratio_df$Proportion[long_ratio_df$value=="Normal"], long_ratio_df$Proportion[long_ratio_df$value=="AAH"])
    writeLines(paste0("Normal-AAH:", normal_aah_ttest$p.value), subtype_proportion_pfile)
    normal_ais_ttest <- t.test(long_ratio_df$Proportion[long_ratio_df$value=="Normal"], long_ratio_df$Proportion[long_ratio_df$value=="AIS"])
    writeLines(paste0("Normal-AIS:", normal_ais_ttest$p.value), subtype_proportion_pfile)
    normal_mia_ttest <- t.test(long_ratio_df$Proportion[long_ratio_df$value=="Normal"], long_ratio_df$Proportion[long_ratio_df$value=="MIA"])
    writeLines(paste0("Normal-MIA:", normal_mia_ttest$p.value), subtype_proportion_pfile)
    normal_adc_ttest <- t.test(long_ratio_df$Proportion[long_ratio_df$value=="Normal"], long_ratio_df$Proportion[long_ratio_df$value=="ADC"])
    writeLines(paste0("Normal-ADC:", normal_adc_ttest$p.value), subtype_proportion_pfile)
    aah_ais_ttest <- t.test(long_ratio_df$Proportion[long_ratio_df$value=="AAH"], long_ratio_df$Proportion[long_ratio_df$value=="AIS"])
    writeLines(paste0("AAH-AIS:", aah_ais_ttest$p.value), subtype_proportion_pfile)
    aah_mia_ttest <- t.test(long_ratio_df$Proportion[long_ratio_df$value=="AAH"], long_ratio_df$Proportion[long_ratio_df$value=="MIA"])
    writeLines(paste0("AAH-MIA:", aah_mia_ttest$p.value), subtype_proportion_pfile)
    aah_adc_ttest <- t.test(long_ratio_df$Proportion[long_ratio_df$value=="AAH"], long_ratio_df$Proportion[long_ratio_df$value=="ADC"])
    writeLines(paste0("AAH-ADC:", aah_adc_ttest$p.value), subtype_proportion_pfile)
    ais_mia_ttest <- t.test(long_ratio_df$Proportion[long_ratio_df$value=="AIS"], long_ratio_df$Proportion[long_ratio_df$value=="MIA"])
    writeLines(paste0("AIS-MIA:", ais_mia_ttest$p.value), subtype_proportion_pfile)
    ais_adc_ttest <- t.test(long_ratio_df$Proportion[long_ratio_df$value=="AIS"], long_ratio_df$Proportion[long_ratio_df$value=="ADC"])
    writeLines(paste0("AIS-ADC:", ais_adc_ttest$p.value), subtype_proportion_pfile)
    mia_adc_ttest <- t.test(long_ratio_df$Proportion[long_ratio_df$value=="MIA"], long_ratio_df$Proportion[long_ratio_df$value=="ADC"])
