...
for (ir in 1:length(roi_vec)) {
    ...
    immune_ratio_vec <- append(immune_ratio_vec, immune_num * 1.0 / ttl_num)
}

# Construct data frame
stage_immune_ratio_df <- data.frame(ROI = roi_vec, Stage = stage_vec, Ratio = immune_ratio_vec)
...
ggplot(stage_immune_ratio_df, aes(x = factor(Stage, level=stage_order), y=Ratio)) + 
    stat_summary(fun.data=MinMeanSEMMax, geom="boxplot", colour="black") + 
    geom_beeswarm(cex = 0.8, size=1.6)   

immune_ratio_plot_path <- file.path(data_root_dir, "NatureFigures", "Fig01", "Stage-Immune-All-Ratio.pdf")
ggsave(filename = immune_ratio_plot_path, device='pdf', width=8, height=8, dpi=300)
while (!is.null(dev.list()))
    dev.off()

immune_ratio_pfile <- file(file.path(data_root_dir, "NatureFigures", "Fig01", "Stage-Immune-All-Ratio-pval.txt"), "w")
# Welch Two Sample t-test p-value
normal_aah_ttest <- t.test(stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="Normal"], stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="AAH"])
writeLines(paste0("Normal-AAH:", normal_aah_ttest$p.value), immune_ratio_pfile)
normal_ais_ttest <- t.test(stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="Normal"], stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="AIS"])
writeLines(paste0("Normal-AIS:", normal_ais_ttest$p.value), immune_ratio_pfile)
normal_mia_ttest <- t.test(stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="Normal"], stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="MIA"])
writeLines(paste0("Normal-MIA:", normal_mia_ttest$p.value), immune_ratio_pfile)
normal_adc_ttest <- t.test(stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="Normal"], stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="ADC"])
writeLines(paste0("Normal-ADC:", normal_adc_ttest$p.value), immune_ratio_pfile)
aah_ais_ttest <- t.test(stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="AAH"], stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="AIS"])
writeLines(paste0("AAH-AIS:", aah_ais_ttest$p.value), immune_ratio_pfile)
aah_mia_ttest <- t.test(stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="AAH"], stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="MIA"])
writeLines(paste0("AAH-MIA:", aah_mia_ttest$p.value), immune_ratio_pfile)
aah_adc_ttest <- t.test(stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="AAH"], stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="ADC"])
writeLines(paste0("AAH-ADC:", aah_adc_ttest$p.value), immune_ratio_pfile)
ais_mia_ttest <- t.test(stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="AIS"], stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="MIA"])
writeLines(paste0("AIS-MIA:", ais_mia_ttest$p.value), immune_ratio_pfile)
ais_adc_ttest <- t.test(stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="AIS"], stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="ADC"])
writeLines(paste0("AIS-ADC:", ais_adc_ttest$p.value), immune_ratio_pfile)
mia_adc_ttest <- t.test(stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="MIA"], stage_immune_ratio_df$Ratio[stage_immune_ratio_df$Stage=="ADC"])
