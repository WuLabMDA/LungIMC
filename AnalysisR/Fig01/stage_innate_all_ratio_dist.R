...
for (ir in 1:length(roi_vec)) {
    ...
    myeloid_ratio_vec <- append(myeloid_ratio_vec, myeloid_num * 1.0 / ttl_num)
}
...
immune_ratio_plot_path <- file.path(data_root_dir, "NatureFigures", "Fig01", "Stage-Innate-All-Ratio.pdf")
ggsave(filename = immune_ratio_plot_path, device='pdf', width=8, height=8, dpi=300)
while (!is.null(dev.list()))
    dev.off()


immune_ratio_pfile <- file(file.path(data_root_dir, "NatureFigures", "Fig01", "Stage-Innate-All-Ratio-pval.txt"), "w")
