...
lymphoid_lst <- c("CD4-T-Cell", "CD8-T-Cell", "T-Reg-Cell", "B-Cell")
...
for (ir in 1:length(roi_vec)) {
    ...
    lymphoid_ratio_vec <- append(lymphoid_ratio_vec, lymphoid_num * 1.0 / immune_num)
}
...
immune_density_plot_path <- file.path(data_root_dir, "NatureFigures", "Fig01", "Stage-Adaptive-Immune-Ratio.pdf")
ggsave(filename = immune_density_plot_path, device='pdf', width=8, height=8, dpi=300)
while (!is.null(dev.list()))
    dev.off()

immune_ratio_pfile <- file(file.path(data_root_dir, "NatureFigures", "Fig01", "Stage-Adaptive-Immune-Ratio-pval.txt"), "w")
