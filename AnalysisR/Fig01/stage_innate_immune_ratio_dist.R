...
myeloid_lst <- c("Macrophage", "Monocyte", "Dendritic-Cell", "Neutrophil", "MDSC", "NK-Cell")
...
for (ir in 1:length(roi_vec)) {
    ...
    myeloid_num <- 0
    for (myeloid_type in myeloid_lst) 
        myeloid_num <- myeloid_num + sum(roi_celltypes == myeloid_type)
    myeloid_ratio_vec <- append(myeloid_ratio_vec, myeloid_num * 1.0 / immune_num)
}

# Construct data frame
stage_myeloid_ratio_df <- data.frame(ROI = roi_vec, Stage = stage_vec, Ratio = myeloid_ratio_vec)
...
ggplot(stage_myeloid_ratio_df, aes(x = factor(Stage, level=stage_order), y=Ratio)) + 
    stat_summary(fun.data=MinMeanSEMMax, geom="boxplot", colour="black") + 
    geom_beeswarm(cex = 0.8, size=1.6)

immune_density_plot_path <- file.path(data_root_dir, "NatureFigures", "Fig01", "Stage-Innate-Immune-Ratio.pdf")
ggsave(filename = immune_density_plot_path, device='pdf', width=8, height=8, dpi=300)
while (!is.null(dev.list()))
    dev.off()


immune_ratio_pfile <- file(file.path(data_root_dir, "NatureFigures", "Fig01", "Stage-Innate-Immune-Ratio-pval.txt"), "w")
