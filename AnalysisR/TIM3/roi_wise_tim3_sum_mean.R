...
tim3_roi_proportion_dir <- file.path(tim3_dir, "CellType-ROI-SumMean")
...
for (cur_type in unique_cell_types) {
  ...
  for (cur_roi in unique_rois) {
    ...
    cur_roi_tim3_sum <- sum(cur_roi_tim3_vals) / (length(cur_roi_tim3_vals[cur_roi_tim3_vals>0.0]) + 0.01)
    tim3_sum_lst <- append(tim3_sum_lst, cur_roi_tim3_sum)
  }
  roi_sum_df <- data.frame(SumMean=tim3_sum_lst, Stage=stage_lst)
  roi_sum_df$Stage <- factor(roi_sum_df$Stage, levels = c("Normal", "AAH", "AIS", "MIA", "ADC"))
  ggplot(roi_sum_df, aes(x = Stage, y = SumMean, colour = Stage)) + 
    geom_boxplot() +
    # geom_violin() + 
    ...
    ggtitle(paste0(cur_type, " - SumMean"))
  
  # save output 
  pw_pval_res <- pairwise.wilcox.test(roi_sum_df$SumMean, roi_sum_df$Stage, p.adjust.method = "bonferroni") 
  ...
  # calcualte p-val
  pval <- kruskal.test(SumMean ~ Stage, data = roi_sum_df) 
