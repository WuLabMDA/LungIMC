...
for ii = 1:group_num % Loops through each slide
    ...
    for rr = 1:num_rois
        ...
        % sum 
        mem_sum = nak_img + b2m_img;
        % normalize
        max_val = prctile(mem_sum(:), 99);
        min_val = prctile(mem_sum(:), 1);
        mem_sum(mem_sum > max_val) = max_val;
        mem_sum(mem_sum < min_val) = min_val;
        mem_img = uint8(255.0 * (mem_sum - min_val) / (max_val - min_val));
        imwrite(mem_img, fullfile(seg_roi_dir, 'NaK_B2M_Sum.tif'));
    end
end
