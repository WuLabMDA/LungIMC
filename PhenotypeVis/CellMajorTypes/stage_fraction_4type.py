...
if __name__ == "__main__":
    ...
    for cur_stage in stage_lst:
        ...
        stromal_ratio = len([ele for ele in cur_cell_types if  ele == "Stromal"]) * 1.0 / len(cur_cell_types)
        stromal_lst.append(stromal_ratio)
        other_immune_ratio = len([ele for ele in cur_cell_types if ele == "Other Immune"]) * 1.0 / len(cur_cell_types)
        other_immune_lst.append(other_immune_ratio)
        immune_ratio = len([ele for ele in cur_cell_types if  ele == "Immune"]) * 1.0 / len(cur_cell_types)
        immune_lst.append(immune_ratio)
        epithelial_ratio = len([ele for ele in cur_cell_types if ele == "Epithelial"]) * 1.0 / len(cur_cell_types)
    ...
    type_stage_df.plot(x = "Stage", kind = "bar", stacked = True, title = "Percentage Stacked Bar Plot")
    plot_name = "major_4type_stage_fraction"
