...
if __name__ == "__main__":
    ...
    for cur_lesion in lesion_roi_dict.keys():
        ...
        for rind in np.arange(args.rand_times):
            rand_inds = []
            while len(rand_inds) != args.rand_roi_num:
                rand_roi = random.choice(lesion_roi_lst)
                if rand_roi in roi_names:
                    rand_inds.append(roi_names.index(rand_roi))
            ...
            kernel_rows = np.sum(kernel_mat, axis=0)
    ...
    bootstrap_lesion_fea_path = os.path.join(slide_agg_dir, "lesion_bmax_feas.csv")
